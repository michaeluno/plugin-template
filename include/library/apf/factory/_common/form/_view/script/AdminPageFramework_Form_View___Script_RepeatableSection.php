...
class PluginTemplate_AdminPageFramework_Form_View___Script_RepeatableSection extends PluginTemplate_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
     ...
     */
    $.fn.updatePluginTemplate_AdminPageFrameworkRepeatableSections = function( aSettings ) {
    
        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.plugin-template-sections' ).attr( 'id' );
    
        // Store the sections specific options in an array.
        if ( ! $.fn.aPluginTemplate_AdminPageFrameworkRepeatableSectionsOptions ) {
            $.fn.aPluginTemplate_AdminPageFrameworkRepeatableSectionsOptions = [];
        }
        if ( ! $.fn.aPluginTemplate_AdminPageFrameworkRepeatableSectionsOptions.hasOwnProperty( _sSectionsContainerID ) ) {
            $.fn.aPluginTemplate_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ] = $.extend(
                {    
        ...
        var _aOptions = $.fn.aPluginTemplate_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ];
        ...
        $( _oThis ).find( '.repeatable-section-add-button' ).click( function() {
            ...
            $( this ).addPluginTemplate_AdminPageFrameworkRepeatableSection();
        ...
        $( _oThis ).find( '.repeatable-section-remove-button' ).click( function() {
            $( this ).removePluginTemplate_AdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });     
        
        // If the number of sections is less than the set minimum value, add sections. 
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.plugin-template-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.plugin-template-section' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentSectionCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentSectionCount ) > 0 ) {     
                $( '#' + _sSectionID ).addPluginTemplate_AdminPageFrameworkRepeatableSection( _sSectionID );  
            }
        }
     ...
     */    
    $.fn.addPluginTemplate_AdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        
        // Local variables
        if ( 'undefined' === typeof sSectionContainerID ) {
            var sSectionContainerID = $( this ).closest( '.plugin-template-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.plugin-template-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.plugin-template-section-tabs-contents' )
            .children( '.plugin-template-section-tabs' )
            .first();
            
        var _iSectionIndex          = nodeSectionsContainer.attr( 'data-largest_index' );
        
        var _iFadein                = $.fn.aPluginTemplate_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadein' ];
        var _iFadeout               = $.fn.aPluginTemplate_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadeout' ];
        
        // If the set maximum number of sections already exists, do not add.
        var _sMaxNumberOfSections   = $.fn.aPluginTemplate_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'max' ];
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.plugin-template-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.plugin-template-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.plugin-template-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.plugin-template-section-title' ).not( '.plugin-template-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enablePluginTemplate_AdminPageFrameworkCollapsibleButton ){ 
            nodeNewSection.find( '.plugin-template-collapsible-sections-title, .plugin-template-collapsible-section-title' ).enablePluginTemplate_AdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.plugin-template-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).updatePluginTemplate_AdminPageFrameworkRepeatableFields();
                                        
            // Callback the registered callback functions.
            
            // @deprecated 3.8.8 Kept for backward compatibility.
            $( this ).trigger( 
                'plugin-template_added_repeatable_field', 
                [
            ...
            // 3.8.8 
            $( this ).trigger( 
                'plugin-template_repeated_field', 
                [
                    1, // call type, 0: repeatable fields, 1: repeatable sections, 
                    jQuery( nodeNewSection ).closest( '.plugin-template-sections' )    // model container
                ]
            );              
        ...
        nodeNewSection.updatePluginTemplate_AdminPageFrameworkRepeatableSections();    
        
        // Rebind sortable fields - iterate sortable fields containers.
        nodeNewSection.find( '.plugin-template-fields.sortable' ).each( function() {
            $( this ).enablePluginTemplate_AdminPageFrameworkSortableFields();
        });
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.plugin-template-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.plugin-template-fieldrow, .plugin-template-fieldset, .plugin-template-fields, .plugin-template-field, table.form-table, input,textarea,select' )
                .incrementAttribute( 
                    'id', 
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.plugin-template-fields' ).incrementAttribute( 
                'data-field_tag_id_model',
                iSectionsCount,
                _sSectionIDModel
            );
            $( oElement ).find( '.plugin-template-fields' ).incrementAttributes( 
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.plugin-template-fieldset' ).incrementAttribute( 
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
     ...
     */
    $.fn.removePluginTemplate_AdminPageFrameworkRepeatableSection = function() {
        
        // Local variables - preparing to remove the sections container element.
        var nodeSectionContainer    = $( this ).closest( '.plugin-template-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.plugin-template-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.plugin-template-section-tabs-contents' )
            .children( '.plugin-template-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.plugin-template-section-tab' );
        ...
        var _sMinNumberOfSections = $.fn.aPluginTemplate_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'min' ];
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.plugin-template-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.plugin-template-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() { 
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.plugin-template-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.plugin-template-section-title' ).first().show();
                }                
            }            
...
var _iSectionIndex = _iIterationIndex;
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.plugin-template-field' ).each( function( iFieldIndex ) {    
...
JAVASCRIPTS;
    ...
    static public function getEnabler($sContainerTagID, $iSectionCount, $asArguments, $oMsg) {
        ...
        new self($oMsg);
        $_oFormatter = new PluginTemplate_AdminPageFramework_Form_Model___Format_RepeatableSection($asArguments, $oMsg);
        $_aArguments = $_oFormatter->get();
        $_sButtons = "<div class='plugin-template-repeatable-section-buttons-outer-container'>" . "<div " . self::___getContainerAttributes($_aArguments, $oMsg) . ' >' . "<a " . self::___getRemoveButtonAttributes($sContainerTagID, $oMsg, $iSectionCount) . ">-</a>" . "<a " . self::___getAddButtonAttributes($sContainerTagID, $oMsg, $_aArguments) . ">+</a>" . "</div>" . "</div>" . PluginTemplate_AdminPageFramework_Form_Utility::getModalForDisabledRepeatableElement('repeatable_section_disabled_' . $sContainerTagID, $_aArguments['disabled']);
...
jQuery( document ).ready( function() {
    ...
    // Adds the buttons
    jQuery( '#{$sContainerTagID} .plugin-template-section-caption' ).each( function(){
        
        jQuery( this ).show();
        
        var _oButtons = jQuery( $_sButtonsHTML );
        if ( jQuery( this ).children( '.plugin-template-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
            _oButtons.addClass( 'section_title_field_sibling' );
        }
        var _oCollapsibleSectionTitle = jQuery( this ).find( '.plugin-template-collapsible-section-title' );
    ...
    // Update the fields     
    jQuery( '#{$sContainerTagID}' ).updatePluginTemplate_AdminPageFrameworkRepeatableSections( $_aJSArray ); 
});            
JAVASCRIPTS;
        return "<script type='text/javascript' class='plugin-template-section-repeatable-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    static private function ___getContainerAttributes(array $aArguments, $oMsg) {
        $_aAttriubtes = array('class' => self::getClassAttribute('plugin-template-repeatable-section-buttons', empty($aArguments['disabled']) ? '' : 'disabled'),);
