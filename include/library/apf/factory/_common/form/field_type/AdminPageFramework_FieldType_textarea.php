...
class PluginTemplate_AdminPageFramework_FieldType_textarea extends PluginTemplate_AdminPageFramework_FieldType {
...
jQuery( document ).ready( function(){
    ...
    var updateFoundEditors = function( oElements ) {
        ...
        oElements.each( function( iIndex ) {
            ...
            var oSettings = jQuery().getPluginTemplate_AdminPageFrameworkInputOptions( oWrap.attr( 'data-id' ) );   
    ...
    jQuery().registerPluginTemplate_AdminPageFrameworkCallbacks( {	
        ...
        repeated_field: function( oCloned, aModel ) {
            ...
            var _oSettings = jQuery().getPluginTemplate_AdminPageFrameworkInputOptions( _oWrap.attr( 'data-id' ) );  

            // Elements
            var _oField              = oCloned.closest( '.plugin-template-field' );
        ...
        stopped_sorting_fields : function( oSortedFields, sFieldType, sFieldsTagID, iCallType ) { 
            ...
            // Update the editor.
            setTimeout(function(){
                var _oFields = oSortedFields.children( '.plugin-template-field' );
        ...
        stopped_sorting_sections: function( oSections ) {
            ...
            setTimeout(function(){
                var _oFields = jQuery( oSections ).find( '.plugin-template-field' );
                updateFoundEditors( _oFields );           
            }, 100 );
        ...
        saved_widget : function( oWidget ) { 
            ...
            jQuery( oWidget ).find( '.plugin-template-field' ).each( function( iIndex ) {
                ...
                // Store the settings.
                jQuery().storePluginTemplate_AdminPageFrameworkInputOptions( 
                    oWrap.attr( 'data-id' ), 
                    { 
...
JAVASCRIPTS;
    ...
    protected function getStyles() {
        return ".plugin-template-field-textarea .plugin-template-input-label-string {vertical-align: top;margin-top: 2px;} .plugin-template-field-textarea .wp-core-ui.wp-editor-wrap {margin-bottom: 0.5em;}.plugin-template-field-textarea.plugin-template-field .plugin-template-input-label-container {vertical-align: top; float: left;clear: both;} .postbox .plugin-template-field-textarea .plugin-template-input-label-container {width: 100%;}.plugin-template-field-textarea textarea {max-width: 100%;}";
    }
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField) {
        ...
        $_sClassSelector = $_bIsArray ? 'plugin-template-field-textarea-multiple-labels' : '';
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $sKey, $aField['label']), "<div class='plugin-template-input-label-container {$_sClassSelector}'>", "<label for='" . $aField['attributes']['id'] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField['label']), $_sLabel ? "<span " . $this->getLabelContainerAttributes($aField, 'plugin-template-input-label-string') . ">" . $_sLabel . "</span>" : '', $this->_getEditor($aField), $this->getElementByLabel($aField['after_input'], $sKey, $aField['label']), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $aField['label']),);
        return implode('', $_aOutput);
    }
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='plugin-template-textarea-enabler'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
}
